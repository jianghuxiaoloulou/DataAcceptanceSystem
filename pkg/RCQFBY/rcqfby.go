...
func GetApplyData(hospital global.HospitalConfig, object global.ApplyDicomData) {
	var sql string
	if hospital.PacsDBType.String == "mysql" {
		global.Logger.Debug("开始通过mysql视图获取申请单数据：")
		...
		study_doctor_id,study_doctor_code,study_doctor_name,study_time,assist_doctor_id,assist_doctor_code,assist_doctor_name,
	...
	} else if hospital.PacsDBType.String == "mssql" {
...
func GetDicomData(db *sql.DB, hospital global.HospitalConfig, accessionnumber string) {
	var sql string
	if hospital.PacsDBType.String == "mysql" {
		global.Logger.Debug("开始通过mysql视图获取DICOM数据：")
...
// 通过存储过程回写数据到妇保院
func WriteBackProc(data global.RcqfbtApplyData) bool {
...
func ReportDataWriteBack(applyid string) {
	...
	flag := WriteBackProc(objdata.Data)
	data := global.WriteBackStatus{
		RegisterID: objdata.Data.RegisterId,
	}
	if flag {
		data.Status = 1
		QYPACSWriteBackStatus(data)
	} else {
		data.Status = 0
		QYPACSWriteBackStatus(data)
	}
...
// 调用QYPACS状态通知接口
func QYPACSWriteBackStatus(data global.WriteBackStatus) {
	global.Logger.Debug("开始执行QYPAC状态同步接口, ", data)
	reqdata, err := json.Marshal(data)
	if err != nil {
		global.Logger.Error(err)
		return
	...
	url := global.SystemData.QYPacsInterfaceUrl
	url += "//"
	url += "ms-qypacs//v1//outside//report//writeBack//status"
	global.Logger.Debug("操作的URL: ", url)
	resp, err := http.Post(url, "application/json", bytes.NewBuffer(reqdata))
	if err != nil {
		global.Logger.Error(err)
...
func SendRemoteDiagnoseApplyData(applyid string) {
	...
	for _, body := range objdata.Data.BodyPartList {
		for _, item := range body.ProjectList {
			// 获取映射的检查项目
			hosItem := model.GetHospitialItemInfo(objdata.Data.CenterHospitalId, item.ProjectId)
			flpitem := global.FLP_CheckItem{
				ProcedureCode: hosItem.HospitalProjectCode.String,
				CheckingItem:  hosItem.HospitalProjectName.String,
